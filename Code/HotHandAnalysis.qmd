...
create_top30_3pa_datasets <- function(start_year = 2001, end_year = 2024) {
  for (year in start_year:end_year) {
    ...
    if (exists(totals_name, envir = .GlobalEnv)) {
      ...
      top30 <- data |>
        dplyr::select(Player, Age, Team, Pos, `3PA`, `3P`) |>
...
Gathering Player ID's to make filtering easier

```{r}
nba_pbp2024 <- hoopR::load_nba_pbp(season = 2024) #%>%
  #filter(grepl("Tim Hardaway", text, ignore.case = TRUE))


# Creating dataset to help with gathering player IDs
id_lookup_01_02 <- nba_pbp2002 %>%
  mutate(
    text = str_remove(text, "\\(.*")
  ) %>%
  filter(
    shooting_play == TRUE,
    str_detect(text, paste(players01_02Ind, collapse = "|"))
  ) %>%
  mutate(
    player = str_extract(text, paste(players01_02Ind, collapse = "|"))
  ) %>%
...
```{r}
# gathering player IDs
get_player_ids <- function(id_lookup_df, min_n = 100) {
...
ids01_02 <- get_player_ids(id_lookup_01_02, min_n = 100)
```

```{r}
id_lookup_01_02_summary <- id_lookup_01_02 %>%
  count(player, athlete_id_1, name = "count") %>%
  arrange(player, desc(count))
...
all_ids_df <- bind_rows(
  lapply(names(ids_list), function(season) {
    data.frame(
      season = season,
      athlete_id = ids_list[[season]]
    )
  }),
  .id = "season_index"  # optional, removes automatic row numbering
) %>%
  select(-season_index) %>%  # remove if not needed
  arrange(as.integer(season), athlete_id)

```

```{r}
# Did 01-02 season manually
ids01_02 <- c(349,881,194,42,63,913,
              26,714,566,24,246,609,
              402,429,841,933,800,868,
              264,662,813,703,469,883,
              9,552,592,301,136,655)

# Did 24-25 season manually
ids24_25 <- c(4594268,3975,3907822,4065648,3078576,4395725,
              3992,4437244,4277905,3908809,4066354,4277956,
              4351851,4395651,4396993,6475,3136193,2990984,
              3064440,4066457,4396907,4432816,6606,4433627,
              4278104,3157465,3945274,4592479,4431678,4914336)
```

```{r}
#season 2024-25
nba_pbp2025 <- hoopR::load_nba_pbp(season = 2025) |>
  filter(grepl("Anthony Edwards", text, ignore.case = TRUE)) |>
  mutate(distance = as.numeric(str_extract(text, "\\d+(?=-foot)"))) |>
  filter(score_value != 2) |>
  filter(is.na(distance)) |>
  filter(!grepl("rebound", type_text, ignore.case = TRUE),
         !grepl("turnover", type_text, ignore.case = TRUE),
         !grepl("layup", type_text, ignore.case = TRUE),
         !grepl("foul", type_text, ignore.case = TRUE),
         !grepl("free throw", type_text, ignore.case = TRUE),
         !grepl("substitution", type_text, ignore.case = TRUE),
         !grepl("jumpball", type_text, ignore.case = TRUE),
         !grepl("tip", type_text, ignore.case = TRUE),
         !grepl("kicked", type_text, ignore.case = TRUE),
         !grepl("jumpball", type_text, ignore.case = TRUE),
         !grepl("charge", type_text, ignore.case = TRUE),
         !grepl("goaltending", type_text, ignore.case = TRUE),
         !grepl("dunk", type_text, ignore.case = TRUE),
         !grepl("ejection", type_text, ignore.case = TRUE),
         !grepl("lane", type_text, ignore.case = TRUE),
         !grepl("traveling", type_text, ignore.case = TRUE),
         !grepl("hook", type_text, ignore.case = TRUE),
         !grepl("two point", text, ignore.case = TRUE),
         )
```
...
nba_pbp2025 <- nba_pbp2025 |>
  filter(athlete_id_1 %in% ids24_25) |>
  mutate(distance = as.numeric(str_extract(text, "\\d+(?=-foot)"))) |>
  #mutate(text = gsub("\\s*\\(.*", "", text)) |>   #remove everything from '(' onward
  filter(grepl("three point", text, ignore.case = TRUE) | 
        (grepl("running pullup", text, ignore.case = TRUE) & distance >= 22) |
        (grepl("driving floating", text, ignore.case = TRUE) & distance >= 22) |
         distance >= 22 |
         score_value == 3
         )|>
...
`ShaedonSharpe_shots_24-25` <- ifelse(ShaedonSharpe2025$scoring_play, 1, 0)
...
players17_18Ind[players17_18Ind == "C.J. Miles"] <- "CJ Miles"
players15_16Ind[players15_16Ind == "CJ McCollum"] <- "C.J. McCollum"
players15_16Ind[players15_16Ind == "JJ Redick"] <- "J.J. Redick"
players14_15Ind[players14_15Ind == "JJ Redick"] <- "J.J. Redick"
players14_15Ind[players14_15Ind == "Lou Williams"] <- "Louis Williams"
players12_13Ind[players12_13Ind == "JJ Redick"] <- "J.J. Redick"
players11_12Ind[players11_12Ind == "JJ Redick"] <- "J.J. Redick"
players08_09Ind[players08_09Ind == "Metta World Peace"] <- "Ron Artest"
players01_02Ind[players01_02Ind == "Peja Stojakovic"] <- "Predrag Stojakovic"
```
...
```{r}
make_season_shot_vectors_ids <- function(
  season_year,
  athlete_ids,       # vector of athlete_id_1 for this season
  id_lookup_df       # dataframe with player names and athlete IDs
) {
  
  # Load play-by-play
  pbp <- hoopR::load_nba_pbp(season = season_year)
  
  # Filter to regular season shooting plays
  pbp_shots <- pbp %>%
    filter(
  ...
  # Merge in id_lookup_df to attach athlete_id to each shot
  # id_lookup_df should have columns: player, athlete_id_1
  pbp_shots <- pbp_shots %>%
    left_join(id_lookup_df, by = c("athlete_id_1"))
  
  # Create a list to store shot vectors
  shot_vectors <- vector("list", length(athlete_ids))
  names(shot_vectors) <- athlete_ids
  
  # Loop through each athlete_id
  for (aid in athlete_ids) {
    
    # Get all shots by this athlete
    athlete_shots <- pbp_shots %>%
      filter(athlete_id_1 == aid)
    
    # 0/1 vector: 1 = made shot, 0 = missed
    shot_vec <- ifelse(athlete_shots$scoring_play, 1L, 0L)
    
    shot_vectors[[as.character(aid)]] <- shot_vec
  }
...
all_shot_vectors_new <- vector("list", length(season_years))
names(all_shot_vectors_new) <- season_years

for (yr in season_years) {
  all_shot_vectors_new[[as.character(yr)]] <-
    make_season_shot_vectors_ids(yr, player_lists[[as.character(yr)]])
...
appendix_table <- season_level_results %>%
  rowwise() %>%
  mutate(
    n_successes = sum(all_shot_vectors[[as.character(season)]][[player]]),
...
```{r}
opportunity_df_all <- opportunity_df_all %>%
  mutate(
    season_end = as.numeric(str_extract(season, "(?<=-)\\d{2}$")) + 2000
  )
...
ggplot(opportunity_df_all, aes(x = as.factor(season_end), y = opportunities)) +
...
Average Number of Hot Hand Opportunities by Season

```{r}
avg_opps_by_season <- opportunity_df_all %>%
  group_by(season_end) %>%
  summarise(
    avg_opportunities = mean(opportunities, na.rm = TRUE),
    total_opportunities = sum(opportunities, na.rm = TRUE),
    n_players = n(),
    .groups = "drop"
  )

ggplot(avg_opps_by_season, aes(x = season_end, y = avg_opportunities)) +
  geom_point(size = 2) +
  geom_line(alpha = 0.4) +
  geom_smooth(se = FALSE) +
  labs(
    title = "Average Hot Hand Opportunities by Season",
    x = "Season",
    y = "Average Opportunities per Player"
  ) +
  theme_minimal() +
  scale_x_continuous(
    breaks = seq(2002, 2025, by = 1),  # tick every year
    labels = seq(2002, 2025, by = 1)   # labels match ticks
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
  )
...
```{r}
# Aggregate by season
season_fg_pct <- opportunity_df_all %>%
  group_by(season) %>%
  summarise(
    total_makes = sum(opportunity_makes, na.rm = TRUE),
    total_opps  = sum(opportunities, na.rm = TRUE),
    fg_pct      = total_makes / total_opps
  ) %>%
  ungroup()
```
...
3-Point Seasonal Field Goal Percentage Following Hot Hand Opportunity

```{r}
ggplot(season_fg_pct, aes(x = season_end, y = fg_pct)) +
  geom_point(color = "black", size = 2) +
  geom_line(alpha = 0.4) +
  geom_smooth(method = "loess", se = T) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "3-Point Seasonal Field Goal Percentage During Hot Hand Opportunity",
    subtitle = "Among Top 30 Shooters",
    x = "Season",
    y = "3P%"
  ) +
  theme_minimal() +
...
-   Look at player's last attempts of game and first attempts of game to see if the percentages are similar.

Getting combined 3P% for the top 30 shooters for each season:

```{r}
# Getting combined 3P% for the top 30 shooters for each season: 
get_season_3p_pct <- function(df, season_label) {
  
  data.frame(
    Season = season_label,
    three_p_pct = sum(df$`3P`, na.rm = TRUE) / sum(df$`3PA`, na.rm = TRUE)
  )
}
```
...
```{r}
season_dfs <- list(
  "2001-02" = `players01-02_top30_3PA`,
  "2002-03" = `players02-03_top30_3PA`,
  "2003-04" = `players03-04_top30_3PA`,
  "2004-05" = `players04-05_top30_3PA`,
  "2005-06" = `players05-06_top30_3PA`,
  "2006-07" = `players06-07_top30_3PA`,
  "2007-08" = `players07-08_top30_3PA`,
  "2008-09" = `players08-09_top30_3PA`,
  "2009-10" = `players09-10_top30_3PA`,
  "2010-11" = `players10-11_top30_3PA`,
  "2011-12" = `players11-12_top30_3PA`,
  "2012-13" = `players12-13_top30_3PA`,
  "2013-14" = `players13-14_top30_3PA`,
  "2014-15" = `players14-15_top30_3PA`,
  "2015-16" = `players15-16_top30_3PA`,
  "2016-17" = `players16-17_top30_3PA`,
  "2017-18" = `players17-18_top30_3PA`,
  "2018-19" = `players18-19_top30_3PA`,
  "2019-20" = `players19-20_top30_3PA`,
  "2020-21" = `players20-21_top30_3PA`,
  "2021-22" = `players21-22_top30_3PA`,
  "2022-23" = `players22-23_top30_3PA`,
  "2023-24" = `players23-24_top30_3PA`,
  "2024-25" = `players24-25_top30_3PA`
)

```

Creating dataset that has 3P% by year for top 30 shooters combined

```{r}
three_pt_pct_by_season <- bind_rows(
  lapply(names(season_dfs), function(season) {
    get_season_3p_pct(season_dfs[[season]], season)
  })
)
```

Joining season_fg_pct and three_pt_pct_by_season to add baseline 3P%

```{r}
season_fg_pct <- left_join(season_fg_pct, three_pt_pct_by_season, by = "Season")
```

3-Point Seasonal Field Goal Percentage Following Hot Hand Opportunity with Baseline 3P%

```{r}
season_fg_long <- season_fg_pct %>%
  pivot_longer(
    cols = c(fg_pct, three_p_pct),
    names_to = "type",
    values_to = "pct"
  ) %>%
  mutate(
    type = recode(type,
                  fg_pct = "Hot-hand 3P%",
                  three_p_pct = "Baseline 3P%")
  )

```

```{r}
season_fg_long <- season_fg_long %>%
  mutate(
    season_end = as.numeric(str_extract(Season, "(?<=-)\\d{2}$")) + 2000
  )
```
...
```{r}
ggplot(season_fg_long, aes(x = season_end, y = pct, color = type)) +
  geom_point(size = 2) +
  geom_line(alpha = 0.4) +
  geom_smooth(method = "loess", se = T) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title = "Hot-Hand vs Baseline 3P% by Season",
...
Difference between Hot-Hand 3P% and Baseline 3P%

```{r}
season_fg_pct <- season_fg_pct %>%
  mutate(diff = fg_pct - three_p_pct)

ggplot(season_fg_pct, aes(x = season_end, y = diff)) +
...
-   If there is a streak, calculate how long those streaks are (3, 4, 5, etc.) and see how that impacts a potential bias / difference in hot-hand 3P% vs Baseline 3P%
