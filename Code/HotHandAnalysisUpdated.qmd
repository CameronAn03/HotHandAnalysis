...
```{r}
# trying to replicate Miller and Sanjurjo's biaschart using MCMC?
...
results <- expand.grid(n = n_values, k = k_values) %>%
  rowwise() %>%
  mutate(
    expected_value = simulate_expectation(n, k, p_hat)
  ) %>%
  ungroup()
```

```{r}
ggplot(results, aes(x = n, y = expected_value, color = factor(k))) +
  geom_line(linewidth = 1.2) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Expected Value of Hot Hand Statistic Under Independence",
    subtitle = paste0("Simulated with p = ", round(p_hat, 3)),
    x = "Number of Shots in Sequence (n)",
    y = expression(E[hat(p)[opp] - hat(p)[others]]),
    color = "k (streak length)"
  ) +
