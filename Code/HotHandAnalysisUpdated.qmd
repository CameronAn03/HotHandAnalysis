...
```{r}
ggplot(results, aes(x = n, y = expected_value, color = factor(sprintf("%.2f", p)))) +
...
stratified_sample %>%
  kable(
    ...
    Table _: Observed and Permutation Statistics for Games with Short Sequences (n = 10-20)</div>",
    digits = 6,
    align = 'c'
  ) %>%
  kable_styling(
...
appendix_table_top30 %>%
  kable(
    format = "html",
    caption = "<div style='text-align:center; font-size:18px; font-weight:bold; color:black'>
    Top 30 Player–Seasons with Largest Degree of Hot Hand using k = 3 (sorted by p-value of the permuation test)</div>",
    digits = 6,
    align = 'c'
  ) %>%
  kable_styling(
