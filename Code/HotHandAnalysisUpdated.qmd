...
results <- expand.grid(n = n_values, p = p_values) %>%
  rowwise() %>%
  mutate(
    expected_value = simulate_expectation(n_values, k_values, p_values)
  ) %>%
  ungroup()
```
...
```{r}
obs_beta <- fixef(mixed.opportunities)$cond["is_opportunity"]
```

```{r}
# permutation function

compute_opportunity <- function(y) {
  as.integer(
    dplyr::lag(y, 1, default = 0) == 1 &
    dplyr::lag(y, 2, default = 0) == 1 &
    dplyr::lag(y, 3, default = 0) == 1
...
permute_once <- function(df) {
  df_perm <- df %>%
    group_by(game_id) %>%
    arrange(shot_number, .by_group = TRUE) %>%
    mutate(make = sample(make)) %>%   # shuffle within game
    mutate(is_opportunity = compute_opportunity(make)) %>%
    ungroup()
  ...
  if (is.na(beta) || !is.finite(beta)) return(NA_real_)
...
n_perm <- 100  # start small, increase later

perm_betas <- replicate(n_perm, permute_once(shot_level_data_seq))
...
```{r}
# bias estimate
mean_perm <- mean(perm_betas)

# bias-adjusted coefficient
beta_adj <- obs_beta - mean_perm

# Permutation p-value
p_val <- mean(perm_betas >= obs_beta)
```
...
```{r}
# Visualization
hist(perm_betas, main = "Null Distribution of Opportunity Coefficient")
abline(v = obs_beta, col = "red", lwd = 2)
```
