...
All data used in this section were obtained from publicly available NBA statistics obtained from Basketball Reference [@basketball_reference]. 

The most notable example of the evolution of offensive strategy is Stephen Curry’s 2015-16 unanimous MVP season, in which he attempted a then-record 886 three-pointers. Figure 2.1 below displays an increasing trend in the percentage of total field goal attempts (FGA) that were three-point attempts (3PA) after the 2015-16 regular season. Field goal attempts are defined as "any live-ball throw, shot, or tap---including layups and dunks---directed at a player's own basket to score, excluding free throws" [@nba_rulebook]. The season variable is encoded as the ending year of each NBA season (e.g., 2016 corresponds to the 2015-16 season).
...
Figure 2.3 displays the league-average field goal attempts (FGA) per game at the team level increasing at a commensurate rate to the pace of play from approximately the 2015-16 to the 2019-20 regular season.
...
Ross (2017) lists various *streak statistics* that can be used to analyze finite sequences of binary outcomes. A streak statistic is a measure designed to quantify the “streakiness” of a given shot sequence. The streak statistic used in my analysis is defined below:
...
This statistic was chosen because it provides a direct comparison of shooting performance in a “hot” state against shooting performance outside of that state. Many individuals expect this streak statistic to equal zero under independence because players should perform similarly following streaks of success and not following streaks of success. However, as discussed earlier, this expectation is biased downward in finite samples. Figure 2.10 displays the distributions of the streak statistic for individual games across the 10 most recent NBA seasons, with the black points representing the mean of the streak statistic for a given season.

```{r}
#| label: streak-statistic-ridge-figure
#| echo: false
#| fig-cap: "Streak Statistic Distributions by Season."
#| message: false

# Compute season means
season_means_unadjusted <- all_games_analysis %>%
  group_by(season_end) %>%
  summarise(mean_diff = mean(diff, na.rm = TRUE))

# Create factor levels for y-axis
season_levels_unadjusted <- factor(sort(unique(all_games_analysis$season_end)))
...
all_games_analysis %>%
  ...
  geom_density_ridges(scale = 1.2, alpha = 0.7, color = "black") +
  geom_point(
    data = season_means_unadjusted,
    aes(
      x = mean_diff,
      y = factor(season_end, levels = rev(levels(season_levels_unadjusted)))
    ),
    size = 2
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(x = "Difference in FG% on Opportunities vs. Other Shots", y = "Season"#,
       #title = "Seasonal Differences: FG% on Opportunities vs. Other Shots (Bias-Adjusted)"
       ) +
  scale_fill_viridis_d() +
  scale_x_continuous(limits = c(-1,1),
                     labels = scales::percent_format(accuracy = 1)) # x-axis as percentages

```

The streak statistic means are substantially below 0%. However, these values should be interpreted with caution due to the finite sample bias that underestimates the streak statistic. A permutation testing procedure is applied to the shot sequences to account for this bias.
...
Figure A.1 (Appendix A) displays the permutation test results for the 30 player-seasons exhibiting the strongest evidence of a hot-hand effect.
...
To account for the false discovery rate of hot-hand effects, a Benjamini-Hochberg adjustment was applied to the existing $p$-values. After this adjustment, none of the player-seasons remained statistically significant, providing little evidence of a hot-hand effect after correcting for multiple comparisons.  
...
#| echo: false
#| tbl-cap: "Total FGA per Game per Player."
...
#| echo: false
#| fig-cap: "Total FGA per Game for Qualified Players."
#| message: false
...
The permutation test was run for all shot sequences in the sample. Sequences with zero opportunities were assigned a $p$-value of *NA*. Figure B.1 (Appendix B) contains a sample of 30 games, stratified based on the number of total shots, and the collected statistics through the permutation tests. To visualize the effect of different sample sizes on the null distribution, three games of different sample sizes were collected. The three selected games contained 13, 16, and 20 shots to represent Q1, the median, and Q3, respectively. Each game had a streak statistic of -0.5 for consistency. The observed and null statistics are shown in Table 3.2 below.
...
To examine the results further, the games were categorized by the number of opportunities in their respective sequence in Table 3.3 and Figure 3.7A. The figure shows an overall negative relationship between the number of opportunities within a game and the number of games associated with the number of opportunities. This is expected because qualifying for opportunity shots is increasingly difficult as the number of opportunities increases. The *Proportion (Adj. Diff. > 0)* column is plotted in Figure 3.7B and represents the proportion of games in which the bias-adjusted streak statistic was greater than 0, indicating a positive directional effect in favor of the hot hand.

```{r}
#| label: adj-diff-by-opps-table
#| echo: false
#| tbl-cap: "Proportion of Games with Bias-Adjusted Difference Greater than 0."
...
opp_positive_prop |>
  kable(col.names = c("Opportunities", 
                      "Games", 
                        "Proportion (Adj. Diff. > 0)")) |>
  kable_styling(full_width = F)
```

```{r}
#| label: opportunity-proportion-panel-figure
#| echo: false
#| fig-cap: "(A) Opportunity Distribution. (B) Proportion of Games with Positive Adjusted Difference."
#| message: false
#| warning: false
...
p1 <- ggplot(
  opp_positive_prop,
  aes(x = n_opportunities, y = n_games)
) +
  geom_col() +
  labs(
    x = "Number of Opportunities",
    y = "Number of Games",
    title = "A." #Distribution of Opportunities"
  ) +
  theme_minimal()
...
p2 <- ggplot(
  opp_positive_prop,
  aes(x = n_opportunities, y = prop_adj_diff_gt0)
) +
  geom_line() +
  geom_point(size = 2.5) +
  scale_y_continuous(
  ...
  labs(
    ...
    title = "B." #Proportion of Games with Positive Adjusted Difference"
  ) +
  theme_minimal()
...
**Note**: The baseline category for shot distance is 'close'.
...
coef_table <- data.frame(
  ...
  `p-value` = c("<0.0001", "0.0002", "<0.0001", "<0.0001", "0.0007", "<0.0001")
)
...
**Note**: The *z*-values and *p*-values for the fixed effect coefficients should be interpreted with caution as they are subject to the finite sampling bias described above.
...
Additional limitations arise from modeling assumptions and measurement choices. The hierarchical model assumes independence between shots, which is partially addressed through the inclusion of the opportunity variable. However, although the opportunity variable accounts for some variation associated with shot order, it may not fully capture within-game dynamics that create dependent shot outcomes. Furthermore, defining an opportunity as any shot occurring after three consecutive made shots serves only as a proxy for a “hot” state and may not fully reflect the underlying hot-hand effect. 
...
# Top 30 Player-Seasons with the Strongest Hot-Hand Effect

```{r}
#| label: fig-top-30-seasons
#| echo: false
#| fig-cap: "Top 30 Player-Seasons with the Strongest Hot-Hand Effect."
#| out-height: "0.72\\textheight"
#| out-width: "auto"
#| fig-align: "center"

knitr::include_graphics("Top30SeasonsRotated.png")
```

\clearpage

# Permutation Test Results for 30 Randomly Stratified Games

```{r}
#| label: fig-stratified-games
#| echo: false
#| fig-cap: "Permutation Test Results for 30 Randomly Stratified Games."
#| out-height: "0.72\\textheight"
#| out-width: "auto"
#| fig-align: "center"

knitr::include_graphics("StratifiedGamesRotated.png")
```